                                        ...
                                        <article class="media">
                                            <div class="media-left">
                                                <figure class="image is-64x64">
                                                    <img src="{{ asset('images/icon/task.png') }}" alt="Image">
                                                </figure>
                                            </div>
                                            <div class="media-content">
                                                <div class="content">
                                                    <p class="task">
                                                        <i class="fa fa-user" aria-hidden="true"></i><a href="{{route('tasks.show',$task->slug)}}"> {{$task->user->name}}</a> <small>{{$task->created_at->diffForHumans()}} <i class="fa fa-clock-o" aria-hidden="true"></i> </small>
                                                        <br>
                                                        <a href="{{route('tasks.show',$task->slug)}}">#{{$task->id}} «{{$task->name}}»</a>
                                                    </p>
                                                </div>
                                                <nav class="level is-mobile">
                                                    <div class="level-left">
                                                        <form name="form-favorite" action="{{route('favorite.store')}}" method="post">
                                                            {{csrf_field()}}
                                                            <input type="hidden" value="{{$task->id}}" name="id">
                                                            <a type="submit" class="level-item" onclick="forms['form-favorite'].submit();">
